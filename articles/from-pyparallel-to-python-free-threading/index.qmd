...
me, giving me the initial motivation to even embark on the PyParallel project.
...
what happens.  Again, I was keen on targetting the very narrow domain of an
asynchronous I/O-based TCP/IP socket server, like a HTTP server, where none of
the objects that are created as part of servicing an incoming request need to
persist once we've sent bytes back on the wire to the client.
...
Additionally, you can be assured that for all of the objects you rely upon as
...
*immortal*, as far as you're concerned in your parallel callback.
...
So, for PyParallel, I did the easiest thing I could think of: just give each
parallel thread its own thread-local block allocator.  This allocator would
furnish all memory allocation requests that we intercepted by instrumenting
calls like `PyObject_New` and `PyMem_Malloc`.
...
[Protocol](https://docs.python.org/3/library/asyncio-protocol.html#protocols)
current-day Python objects used by `asyncio`.
...
HTTP server that responded to `/plaintext` with b`Hello, World!`) performance
yielded 132,772 requests per second which put it into first place across all
the platforms I benchmarked.
...
removing the GIL via PyParallel around 2012-2013.  We've reviewed four main
primitives that need to be efficiently solved in order for Python to run
simultaneously on multiple cores as optimally as possible:

 1. Reference counting.
 2. Garbage collection.
 3. Memory allocation.
 4. Memory reclaimation.
