...
```c
// Only the two least significant bits are used; remaining bits always zero.
// 0b00: unlocked
// 0b01: locked
...
typedef struct {
    // ob_tid stores the thread id (or zero). It is also used by the GC
    // and the trashcan mechanism as a linked list pointer and by the GC
    // to store the computed "gc_refs" refcount.
