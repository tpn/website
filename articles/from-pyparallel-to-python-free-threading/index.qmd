...
With the ability to quickly determine if we were a parallel thread in place,
we could instrument all of the thread-sensitive calls to follow the
aforementioned paradigm of:

> If we're a *parallel thread*, do X, if not, do Y.



The viability of this approach is contingent upon the overhead involved in
determining if we're a parallel thread.
...
if we're in a parallel thread by way of the `Py_PXCTX()` macro 
