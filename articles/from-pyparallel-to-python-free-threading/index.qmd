...
As long as we've diverted all memory allocation to a thread-safe alternative
during execution of that callback, we can get away with not doing any reference
counting whatsoever.

#### Memory Allocation

Each parallel thread had its own thread-local block allocator that was
responsible for furnishing all memory allocation requests that we intercepted
by instrumenting calls like `PyObject_New` and `PyMem_Malloc`.
...
How do you free memory allocated in these parallel contexts?  Easy: you don't!
At the end of the callback, when you're back in the Python C code after you
have finished executing the user's `data_received()` callback, for example,
you can just reset the block allocator such that it points to the first chunk
of memory in the first block.  You don't actually need to `free()` anything.
...
How do you handle garbage collection?  Easy: you don't!  As no reference count
mutation takes place during a parallel callback, no reference cycles can be
constructed in the first place, obviating the need to even perform garbage
collection.
...
*completion-oriented* protocol callbacks implemented by the user.
 ...
 - Custom `PxSocket_IoLoop()`.
 - Highly-optimized TCP/IP socket server "virtual machine".
 - Integrated `TransmitFile` support.
 - Summary: the `PxSocket_IoLoop()` plus negligible-overhead of our
   instrumented thread-sensitive calls is what led to PyParallel's performance.
...
which was a nichè variant of Python that separated the C call stack from the
...
model, per se.  Or rather, it doesn't suddenly allow you to write your C
programs in a way that mimics a threadpool+overlapped I/O approach you'd use
on Windows.

It also wasn't available in 2012 when I started hacking on PyParallel; it
first appeared in the Linux kernel 5.1 in May 2019.
...
platforms---it extensively used APIs for which there were no
functionally-equivalent counterparts on other platforms.  So, if you were to
try get PyParallel working on Linux or Mac OS X, for example, you'd need to
write a whole bunch of thread pool and synchronization scaffolding, as well
as `PxSocket_IoLoop()`-equivalents using whatever the underlying platform's
optimal socket multiplexing system call was.
...
observing how Python *worked*.  Incredibly valuable experience, by the way.
It's my favorite way of getting up to speed with an unfamiliar code base.
...
My favorite metric was that PyParallel's plaintext performance yielded 132,772
requests per second which put it into first place across all the platforms I
benchmarked.  LWAN, a raw C Linux implementation well-known for its
performance, came in second with 128,126 requests per second.  Granted, I
cheated on my side, leveraging an SSE-accelerated HTTP parser to process the
incoming request, per the `http11 = True` line below:
...
So, we've covered Greg Stein's initial attempts at removing the GIL back in
1996, plus my attempts at removing the GIL limitations without actually
removing the GIL via PyParallel.

Hopefully this provides some context on the type of obstacles that the
free-threading implementation needed to overcome in order to prevail.

And I am happy to report that they absolutely did prevail in overcoming all
of the obstacles that prevented prior work from being viable.  The resulting
Free-Threaded Python 3.13t really is an engineering marvel that took a lot
of effort on many fronts.

I'll spend a little bit of time in this next section covering the
implementation, particularly as it pertains to the things that were done
differently to Greg Stein's work and PyParallel regarding reference counting,
garbage collection, memory, etc.  And then we'll take a look at some
non-trivial examples of real-world problems that can now be tackled
performantly via the [Parallelopedia](https://github.com/tpn/parallelopedia)
example project.
