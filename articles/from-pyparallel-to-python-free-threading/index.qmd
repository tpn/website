...
introduce support for a "no-GIL" ([Global Interpreter
Lock](https://docs.python.org/3/glossary.html#term-global-interpreter-lock)),
*free-threaded* mode, where multiple Python threads can run simultaneously.
...
This is huge.  Seriously.  I can’t overstate how exciting this is.  For
decades, countless attempts have been made to solve this problem---my own
[PyParallel](https://pyparallel.org) attempts included---but until now, no-one
had prevailed in getting a solution integrated into CPython’s core.

I will admit I am biased: I am a Python committer (though not directly involved
in this work), I adore the language, and I spent years obsessing over the dream
of a parallel Python.  It's a topic I'm incredibly passionate about, and this
article will unabashedly proselytize the extraordinary engineering that finally
made a free-threaded Python a reality.
...
implementation and performance on real-world problems.  Problems that, until
now, simply couldn't be tackled with Python alone.

And this is essentially just the first-pass!  Python 3.14 will enable
[PEP-659's Specializing Adaptive
Interpreter](https://docs.python.org/3/whatsnew/3.11.html#whatsnew311-pep659)
functionality to work in free-threaded mode, which will allow new
multi-threaded code to benefit from the speedups delivered in Python 3.11 by
the specialization functionality.  (It was disabled in free-threaded mode
for Python 3.13 as the implementation wasn't thread safe.)
...
tackled with a free-threaded Python.  I'll then introduce a demo project I've
created called [Parallelopedia](https://github.com/tpn/parallelopedia) which
implements a whole bunch of interesting things aimed at demonstrating the cool
stuff you can now do with free-threaded Python.
...
and funding the free-threaded Python efforts, too, again, thanks to their
relationship with Quansight.  So special thanks goes out to Meta for all of
their support with both free-threaded Python, and PyTorch overall.
...
Thank you Meta, Quansight, Travis, and James!
...
In 2008, I was invited to become a Python committer.  I was thrilled.  I had
