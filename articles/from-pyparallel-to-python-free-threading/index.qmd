...
The slowdown to single-threaded code as a result of this patch was
prohibitively brutal, and the patch was never accepted.  Fast-forward 15 years
or so, and David Beazley wrote a detailed post in August 2011 about Greg
Stein's work in an article titled:
[An Inside Look at the GIL Removal Path of Lore](
...
needed.  The reference count was stored in the `ob_refcnt` field.  Incrementing
the reference simply added one to the existing value.  Decrementing the
reference subtracted one and checked to see if the resulting value was zero,
and, if it was, the object would be deallocated via `_Py_Dealloc()`.
...
I hadn't even heard of Python back in 1996---I was only in tenth grade at the
time, and I don't think I even knew what Windows NT was at that point, let
alone Linux or FreeBSD.  Nor did I have any aspirations to be a software
engineer---I was gung-ho on becoming an F/A-18 Hornet fighter pilot in the
Royal Australian Air Force (RAAF).

Getting back on track: Greg's work tackled a lot of other important concepts
like separating global state into per-thread state (something that is still
relied upon today), as well as injecting pooled mutexes into builtin container
types like `list` and `dict` objects.

However, the performance impact to single-threaded code was deemed too great,
and as such, Greg's work never made it into mainline Python.
...
So, as it pertained to reference counting, garbage collection, and memory
allocation and deallocation---all thread-sensitive things that need to be
handled very carefully in a GIL-less Python---with PyParallel, I explored the
notion of: *what if we just didn't do any of that in parallel thread contexts*?

Instead of trying to remove the GIL, let's keep the GIL, and instead,
...
`Py_DECREF()` such that:
 ...
 - If we're not a *parallel thread* (i.e. we're holding the GIL), just do what
   we normally do.
 - If we are a *parallel thread*, do something else.

That's the crux of PyParallel's approach to achieving simultaneous
multi-threading: instrument everything thread sensitive to detect if we're
a *parallel thread*, and do something else if we are.

For reference counting, the *do something else* was literally: do nothing.
...
Larry Hasting's
[Gilectomy](https://github.com/larryhastings/gilectomy/tree/gilectomy).

The hilarious Reddit post by user [/u/corysama](https://reddit.com/u/corysama)
in the thread
[Describe what developing for each console you've developed for is like.](
https://www.reddit.com/r/gamedev/comments/xddlp/comment/c5lg7px/?utm_source=share&utm_medium=web3x&utm_name=web3xcss&utm_term=1&utm_content=share_button).
With regards to PS2 development, the following comment always stood out to me:

> There are so many amazing things you can do, but everything requires
> backflips through invisible blades of segfault.

That absolutely encompasses the first few months of developing PyParallel.  And
then all subsequent months.  Larry Hasting's has a similar sentiment in the
Gilectomy [README](https://github.com/larryhastings/gilectomy/tree/gilectomy),
with the first sentence proudly stating:
