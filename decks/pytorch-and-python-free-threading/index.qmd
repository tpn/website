...
- [PyParallel Website](https://pyparallel.org)
- [PyParallel: How we removed the GIL and exploited all cores (without needing to remove the GIL at all)](https://speakerdeck.com/trent/pyparallel-how-we-removed-the-gil-and-exploited-all-cores)
  - PyData NYC 2013
...
- Cons:
  ...
  - Couldn't (easily) mutate shared state in parallel callback
  - Pure hack fest; crashed a lot, not remotely suitable for production
  - Windows only
:::
...
## Because apparently my solution to all problems is a multi-threaded async I/O HTTP server...

::: {.incremental}
- Quick test to see if you can call `model.generate()` from multiple threads: boring.
- Wouldn't it be cooler to write a multi-threaded async I/O HTTP server
  - And expose `/generate`-esque `GET` endpoints for doing inference
  - And have it stream tokens in real-time
    - Ain't nobody got time to wait for an entire response up-front
...
Launch Server
```bash
python -Xgil=0 -m parallelopedia.http.server --threads 20 --ip 0.0.0.0 --port 4444 --log-level INFO \
...
Piping a manual HTTP GET request via `netcat`:
```bash
% echo -en \
...
```{.bash code-line-numbers="|8|16-17|18-19|20-21|"}
HTTP/1.1 200 OK
